$(document).ready(function () {
  ...
  $("#show").click(function () {
    ...
    if (!check_char($("#words").val(), special_chars)) {
      ...
      $("#error").text(
        "There is a limit of " + max_string + " strings allowed !"
      );
      $("#error").toggle(true);
    } else {
      if (
        str_list.length > 0 &&
        !(str_list.length == 1 && str_list[0] === "")
      ) {
        $("#error").toggle(false);
    ...
    //   });
      $("#viz_area4 .node").mouseenter((e) => {
        ...
        if (suffix === "") {
          $("#documentHighlightedPSingle").html(documents);
        } else {
  ...
  function update(source) {
    ...
    nodeUpdate
      ...
      .attr("r", 10)
      .style("fill", function (d) {
        return d.wasSearched ? "#FFFF00" : d._children ? "#ddd" : "#fff";
      })
      .style("stroke", function (d) {
        return d.wasSearched ? "#000" : d._children ? "#bbb" : colorlist[d.seq];
      });
...
//   });
  ...
  $('#documentHighlightedPSingle').text($('#words').val().split(',')[0].trim())
});
